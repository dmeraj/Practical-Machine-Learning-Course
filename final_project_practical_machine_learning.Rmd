---
title: "Final Project Proctical Machine Learning"
author: "danish"
date: '2022-05-07'
...
```{r}
traincsv <- read_csv("~/Downloads/pml-training.csv")
testcsv <- read_csv("~/Downloads/pml-testing.csv")
dim(traincsv)
dim(testcsv)
```
...
```{r, echo=FALSE}
models <- c("Tree", "GBM", "SVM")
accuracy <- round(c( cmtrees$overall[1], cmgbm$overall[1], cmsvm$overall[1]),3) #accuracy
oos_error <- 1 - accuracy #out of sample error
data.frame(accuracy = accuracy, oos_error = oos_error, row.names = models)
```

**The best model is the Gradient Boosted model, with `r cmgbm$overall[1]` accuracy and `r 1-cmgbm$overall[1]` out of sample error rate. We find that to be a sufficient enough model to use for our test sets. ** 
...
Github Repo: [Github](https://github.com/Decoder3818/Final-Project-Practical-Machine-Learning)
